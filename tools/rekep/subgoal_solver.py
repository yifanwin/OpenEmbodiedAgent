...
def objective(opt_vars,
    ...
    if is_grasp_stage:
        local_axis = np.asarray(grasp_axis_local, dtype=np.float64)
        if np.linalg.norm(local_axis) < 1e-9:
            local_axis = np.array([1.0, 0.0, 0.0], dtype=np.float64)
        local_axis = local_axis / np.linalg.norm(local_axis)
        preferred_dir = np.asarray(grasp_preferred_world_dir, dtype=np.float64)
        if np.linalg.norm(preferred_dir) < 1e-9:
            preferred_dir = np.array([0.0, 0.0, -1.0], dtype=np.float64)
        preferred_dir = preferred_dir / np.linalg.norm(preferred_dir)
        grasp_axis_world = opt_pose_homo[:3, :3] @ local_axis
        grasp_cost = -np.dot(grasp_axis_world, preferred_dir) + 1  # [0, 1]
        grasp_cost = 10.0 * grasp_cost
        debug_dict['grasp_cost'] = grasp_cost
        debug_dict['grasp_axis_world'] = grasp_axis_world
        cost += grasp_cost
...
class SubgoalSolver:
    ...
    def solve(self,
        ...
        grasp_axis_local = np.asarray(self.config.get('grasp_axis_local', [1.0, 0.0, 0.0]), dtype=np.float64)
        grasp_preferred_world_dir = np.asarray(self.config.get('grasp_preferred_world_dir', [0.0, 0.0, -1.0]), dtype=np.float64)
        aux_args = (og_bounds,
                    keypoints_centered,
                    keypoint_movable_mask,
                    goal_constraints,
