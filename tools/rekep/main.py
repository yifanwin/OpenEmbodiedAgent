...
class Main:
    def __init__(self, scene_file, visualize=False):
        ...
        self.max_task_runtime_seconds = float(self.config.get('max_task_runtime_seconds', 1800.0))
        self.max_stalled_iterations = int(self.config.get('max_stalled_iterations', 8))
        # set random seed
        np.random.seed(self.config['seed'])
        torch.manual_seed(self.config['seed'])
        torch.cuda.manual_seed(self.config['seed'])
        # Defer VLM-dependent components until a live query is actually requested.
        self.keypoint_proposer_config = global_config['keypoint_proposer']
        self.constraint_generator_config = global_config['constraint_generator']
        self.keypoint_proposer = None
        self.constraint_generator = None
        # initialize environment
        self.env = ReKepOGEnv(global_config['env'], self.scene_file, verbose=False)
    ...
    def perform_task(self, instruction, rekep_program_dir=None, disturbance_seq=None):
        ...
        if rekep_program_dir is None:
            if self.keypoint_proposer is None:
                self.keypoint_proposer = KeypointProposer(self.keypoint_proposer_config)
            if self.constraint_generator is None:
                self.constraint_generator = ConstraintGenerator(self.constraint_generator_config)
        ...
        # = execute
        # ====================================
        video_path = self._execute(rekep_program_dir, disturbance_seq)
        result = {
            'instruction': instruction,
            'scene_file': self.scene_file,
            'rekep_program_dir': os.path.abspath(rekep_program_dir),
        ...
        for key, name in (
            ...
            candidate = os.path.join(rekep_program_dir, name)
            if os.path.exists(candidate):
                result[key] = os.path.abspath(candidate)
        return result
    ...
    def _execute(self, rekep_program_dir, disturbance_seq=None):
        ...
        while True:
            ...
            if self.max_task_runtime_seconds > 0 and elapsed > self.max_task_runtime_seconds:
                raise TimeoutError(
                    f"ReKep task exceeded max_task_runtime_seconds={self.max_task_runtime_seconds:.1f} at stage {self.stage}"
                )
            ...
            if backtrack:
                ...
                if self.last_sim_step_counter == self.env.step_counter:
                    ...
                    if stalled_iterations >= self.max_stalled_iterations:
                        raise RuntimeError(
                            "ReKep detected repeated no-progress iterations "
                            f"({stalled_iterations} consecutive loops at stage {self.stage})"
                        )
                else:
                    stalled_iterations = 0
    ...
    def _get_grasp_candidate(self):
        grasp_keypoint = self.program_info['grasp_keypoints'][self.stage - 1]
        if grasp_keypoint == -1:
            return None
        return self.env.get_object_by_keypoint(grasp_keypoint)

    def _pose_with_local_offset(self, pose, local_offset):
        new_pose = pose.copy()
        new_pose[:3] += T.quat2mat(pose[3:]) @ np.asarray(local_offset, dtype=float)
        return new_pose

    def _execute_grasp_attempt(self, pregrasp_pose, local_offset, retreat_backoff, reset_before_attempt):
        local_offset = np.asarray(local_offset, dtype=float)
        if local_offset.shape != (3,):
            raise ValueError(f'grasp retry offset must be length-3, got {local_offset}')

        if reset_before_attempt:
            self.env.open_gripper()
            retry_pose = self._pose_with_local_offset(
                pregrasp_pose,
                local_offset + np.array([-retreat_backoff, 0.0, 0.0]),
            )
            self.env.execute_action(
                np.concatenate([retry_pose, [self.env.get_gripper_null_action()]]),
                precise=True,
            )

        grasp_pose = self._pose_with_local_offset(
            pregrasp_pose,
            local_offset + np.array([self.config['grasp_depth'], 0.0, 0.0]),
        )
    ...
    def _execute_grasp_action(self):
        ...
        candidate_obj = self._get_grasp_candidate()
        retry_offsets = self.config.get('grasp_retry_offsets', [[0.0, 0.0, 0.0]])
        settle_time = float(self.config.get('grasp_retry_settle_time', 0.15))
        retreat_backoff = float(self.config.get('grasp_retry_backoff', 0.03))
        last_error = None

        for attempt_idx, offset in enumerate(retry_offsets, start=1):
            offset = np.asarray(offset, dtype=float)
            print(
                f"{bcolors.HEADER}[stage={self.stage}] grasp attempt {attempt_idx}/{len(retry_offsets)} "
                f"with local offset {np.array2string(offset, precision=4)}{bcolors.ENDC}"
            )
            last_error = self._execute_grasp_attempt(
                pregrasp_pose,
                offset,
                retreat_backoff,
                reset_before_attempt=attempt_idx > 1,
            )
            if settle_time > 0:
                self.env.sleep(settle_time)
            if self.env.is_grasping(candidate_obj):
                print(f"{bcolors.OKGREEN}[stage={self.stage}] grasp succeeded on attempt {attempt_idx}{bcolors.ENDC}")
                return
            print(f"{bcolors.WARNING}[stage={self.stage}] grasp attempt {attempt_idx} failed{bcolors.ENDC}")

        pos_error, rot_error = last_error if last_error is not None else (None, None)
        raise RuntimeError(
            "Failed to grasp the target object after "
            f"{len(retry_offsets)} attempts "
            f"(last_pos_error={pos_error}, last_rot_error_deg={np.rad2deg(rot_error) if rot_error is not None else None})"
        )
...
def build_pen_disturbance_seq():
    ...
    return {1: stage1_disturbance_seq, 2: stage2_disturbance_seq, 3: stage3_disturbance_seq}
...
def build_task_list():
    return {
        'pen': {
            ...
            'disturbance_seq': build_pen_disturbance_seq(),
        },
    }
...
def run_named_task(
    ...
    scene_file_override=None,
    rekep_program_dir_override=None,
):
    task_list = build_task_list()
    if task_name not in task_list:
        raise ValueError(f'Unknown task: {task_name}. Available tasks: {sorted(task_list.keys())}')

    if apply_disturbance and not use_cached_query:
        raise ValueError('disturbance sequence is only defined for cached scenario')
    if visualize and _headless_enabled_from_env():
        raise ValueError('visualize=True is incompatible with headless mode; rerun with --no-headless')
    if _headless_enabled_from_env():
        hold_ui_seconds = 0.0
    ...
    rekep_program_dir = (
        ...
        else (task['rekep_program_dir'] if use_cached_query else None)
    )
    disturbance_seq = task.get('disturbance_seq', None) if apply_disturbance else None

    print(
        f"{bcolors.HEADER}Runtime options: headless={_headless_enabled_from_env()} "
        f"visualize={visualize} hold_ui_seconds={hold_ui_seconds:.1f}{bcolors.ENDC}"
    )
    main = Main(scene_file, visualize=visualize)
    try:
        result = main.perform_task(
            instruction,
        ...
        if hold_ui_seconds > 0 and getattr(main, "env", None) is not None:
            print(f'{bcolors.HEADER}Holding Isaac Sim UI open for {hold_ui_seconds:.1f} seconds before shutdown.{bcolors.ENDC}')
            try:
                main.env.sleep(hold_ui_seconds)
            except Exception as exc:
                print(f'{bcolors.WARNING}Failed while holding Isaac Sim UI open: {exc}{bcolors.ENDC}')
        main.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hold_ui_seconds', type=float, default=0.0, help='keep the Isaac Sim window open for a short period before shutdown')
    parser.add_argument('--headless', action=argparse.BooleanOptionalAction, default=None, help='run OmniGibson in headless mode')
    parser.add_argument('--instruction', type=str, default=None, help='override task instruction')
    parser.add_argument('--scene_file', type=str, default=None, help='override scene file')
    parser.add_argument('--rekep_program_dir', type=str, default=None, help='override cached ReKep program directory')
    ...
    run_named_task(
        ...
        hold_ui_seconds=args.hold_ui_seconds,
