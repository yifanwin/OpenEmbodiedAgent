    ...
    def serve(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                elif method == "command_movel":
                    result = self._robot.command_movel(**args)
...
class ZMQClientRobot(Robot):
    ...
    def command_movel(self, pose: np.ndarray, flag_in) -> None:
        request = {
            "method": "command_movel",
            "args": {"pose": pose, "flag_in": flag_in},
        }
        send_message = pickle.dumps(request)
        self._socket.send(send_message)
        result = pickle.loads(self._socket.recv())
    ...
    def command_gripper(self, position, flag_in=None) -> None:
        request = {
            ...
            "args": {"position": position, "flag_in": flag_in},
        }
        send_message = pickle.dumps(request)
        self._socket.send(send_message)
        result = pickle.loads(self._socket.recv())
    ...
    def set_do_status(self, which_do: np.ndarray, arm: str = "left"):
        request = {
            "method": "set_do_status",
            "args": {"which_do": which_do, "arm": arm},
        }
        send_message = pickle.dumps(request)
        start = time.time()
        self._socket.send(send_message)
