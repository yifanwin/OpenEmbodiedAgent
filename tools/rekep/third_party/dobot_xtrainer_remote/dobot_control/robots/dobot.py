...
class DobotRobot(Robot):
    ...
    def command_movel(self, pose: np.ndarray) -> None:
        """Command robot with Cartesian MovL.

        Args:
            pose (np.ndarray): [x, y, z, rx, ry, rz]
        ...
        if len(pose_vec) < 6:
            raise ValueError(f"movel pose length invalid: got {len(pose_vec)}, need >= 6")
        # Dobot dashboard/Move API expects Cartesian mm + degree.
        self.robot.MovL(
            float(pose_vec[0]),
            float(pose_vec[1]),
            float(pose_vec[2]),
    ...
    def command_gripper(self, position, flag_in=None) -> None:
        """Command gripper opening ratio through DobotGripper.

        Args:
            position: opening ratio in [0, 1], where 1 is more open.
            flag_in: ignored for single arm robot.
        """
        assert not self.robot_is_err, f"{self.robot_ip}: error!"
...
def main():
    ...
    dobot.set_freedrive_mode(False)
