...
class GroupSyncRead:
    ...
    def getData(self, scs_id, address, data_length):
            ...
            if scs_id not in self.data_dict or self.data_dict[scs_id] is None:
                raise ValueError("Data for scs_id {} is not available.".format(scs_id))
        

            if data_length == 1:
                return self.data_dict[scs_id][address-self.start_address+1]
            elif data_length == 2:
                return self.ph.scs_makeword(self.data_dict[scs_id][address-self.start_address+2],
